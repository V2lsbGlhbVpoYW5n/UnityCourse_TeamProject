...
public class ComputerWarriorLogic : UnitLogic
{
        ...
        if (attackTarget != null)
        {
            ...
            if (unitLogic != null && !unitLogic.GetDeath())
            {
                ...
                if (colliders.Length > 0)
                {
                    ...
                    {
                        if (col.gameObject.CompareTag("PlayerShooter") ||
                            col.gameObject.CompareTag("PlayerWarrior") ||
                            col.gameObject.CompareTag("PlayerMagician") ||
                            col.gameObject.CompareTag("PlayerInfrastructure"))
                        {
                            col.TryGetComponent(out UnitLogic ulogic);
                            col.TryGetComponent(out PlayerBasement blogic);
